...
async function fetchAgencyData(url) {
  ...
  // Fetch Agency Logo
  const logoData = await page.$$eval(".profile-header__logotype img", (details) => details.map((detail) => detail.getAttribute("src")))
  dataObject.logo = logoData[0].replace("50x50", "100x100")
  ...
  if (packageUrlData != undefined && packageUrlData.length > 0) {
  ...
  if (dataObject.packages_url) {
    ...
    if (packagesTitle != undefined && packagesTitle.length > 0) {
    ...
    if (packagesSummary != undefined && packagesSummary.length > 0) {
